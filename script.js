...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // ─── 10. CONTACT FORM ────────────────────────────────────────────────────
  //
  // Submits to an n8n Webhook node. That workflow (see
  // n8n-workflow-contact-automation.json) does two things automatically:
  //   1. Emails Adel with the new lead's details.
  //   2. Auto-replies to the visitor's own email with a marketing/welcome
  //      message — i.e. lead capture with an instant follow-up.
  //
  ...
  if (contactForm) {
    ...
    contactForm.addEventListener('submit', async (e) => {
      ...
      if (!N8N_WEBHOOK_URL || N8N_WEBHOOK_URL.includes('YOUR-N8N-INSTANCE')) {
        console.error('N8N_WEBHOOK_URL is not set — see the top of the CONTACT FORM section in script.js.');
        showBanner(errorEl);
        return;
      }
